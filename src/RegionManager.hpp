...
public:
    ...
    void create(char* file_path, uint64_t size, bool p = true, bool imm_expand = true){
    ...
    template<class T>
    T* create_for(char* file_path, uint64_t size, bool p = true){
        bool restart = exists_test(file_path);
        // if (regions[cur_idx] != nullptr) {
